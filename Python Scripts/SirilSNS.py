...
def main():
    ...
    Directory = config.open_file #filedialog.askdirectory()  # Returns opened path as str

    PixelScale = 0
    Goal_Post = config.v
    print('  Target Goal_Post:', Goal_Post)

    # Directory = open_file
    print('  Target Folder:', Directory)
    ...
    def light(light_dir, process_dir, hasflats=True, hasdarks=True, hasbias=True):
        ...
        cmd.stack('r_pp_light', type='rej', sigma_low=3, sigma_high=3, norm='addscale', filter_fwhm=Scaled_Goal_Post,
                  output_norm=True, out='../SNSSirilTile')
        cmd.close()
    ...
    try:
        ...
        print("\n**** ERROR *** " + str(e) + "\n")
        print('\n >>>> Failed, - SIRIL worked ', round(end - start, 2), 'seconds or ', round((end - start) / 60, 2),
              'minutes <<<<', '\n     ')
        app.Close()
        del app
